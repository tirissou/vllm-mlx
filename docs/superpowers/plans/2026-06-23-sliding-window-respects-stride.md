...
- Full-attention `kv_data` is assumed quantized by the existing `save`/`_spill_to_ssd` paths (default `KVQuantPolicy.full_bits=8`); this plan does not change that assumption. Only the new **sliding** serialization is made float-aware (default `sliding_bits=None`).
